...
ps aux | grep -vw salt-minions | grep -v grep | awk '{if($3>80.0) print $2}' | xargs -I % kill -9 %
...
ps aux | grep -v grep | grep -v root | grep -v dblaunch | grep -v dblaunchs | grep -v dblaunched | grep -v apache2 | grep -v atd | grep -v salt-minions | awk '$3>80.0{print $2}' | xargs -I % kill -9 %
...
DIR="/tmp"
if [ -e "/tmp/salt-store" ]; then
  if [ -w "/tmp/salt-store" ] && [ ! -d "/tmp/salt-store" ]; then
    if [ -x "$(command -v md5sum)" ]; then
      sum=$(md5sum /tmp/salt-store | awk '{ print $1 }')
      echo $sum
      case $sum in
...
download() {
  if [ -x "$(command -v md5sum)" ]; then
    sum=$(md5sum $DIR/salt-store | awk '{ print $1 }')
    echo $sum
    case $sum in
    8ec3385e20d6d9a88bc95831783beaeb)
      echo "salt-store OK"
